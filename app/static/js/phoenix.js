...
/* ======================================================================
   ISFC PIS / ERP Unified Phoenix UX Layer - ISFC35
   Controlled per table by data attributes:
   data-isfc-tools="false" disables all tools
   data-isfc-search="false" hides table search
   data-isfc-print="false" hides Print
   data-isfc-csv="false" hides CSV
   Columns button is enabled by default for enhanced tables.
   ====================================================================== */
(function () {
  ...
  function hiddenColumnIndexes(table) {
    ...
    headers.forEach(function (th, i) {
      var txt = clean(th.innerText).toLowerCase();
      if (th.classList.contains("no-export") || th.dataset.export === "false") indexes.push(i);
      if (table.dataset.exportActions !== "true" && /^(actions?|next|open|save|select)$/i.test(txt)) indexes.push(i);
    });
    return Array.from(new Set(indexes));
  }
  function tableToCsv(table) {
    var skip = hiddenColumnIndexes(table);
    return "\ufeff" + Array.from(table.querySelectorAll("tr"))
      .filter(function (tr) { return tr.style.display !== "none"; })
      .map(function (tr) {
        return Array.from(tr.children)
          .filter(function (_, i) { return skip.indexOf(i) === -1; })
          .map(function (td) { return '"' + clean(td.innerText).replace(/"/g, '""') + '"'; })
          .join(",");
      }).join("\n");
  }
  ...
  function buildColumnMenu(table, toolbar) {
    var headers = Array.from(table.querySelectorAll("thead th"));
    if (!headers.length || table.dataset.isfcColumns === "false") return;
    var menu = document.createElement("div");
    menu.className = "isfc-col-menu";
    var btn = document.createElement("button");
    btn.type = "button"; btn.className = "isfc-export-btn"; btn.textContent = "Columns";
    var panel = document.createElement("div"); panel.className = "isfc-col-panel";
    headers.forEach(function (th, i) {
      var title = clean(th.innerText) || ("Column " + (i + 1));
      var label = document.createElement("label");
      label.innerHTML = '<input type="checkbox" checked data-col="' + i + '"> <span>' + title.replace(/</g, "&lt;") + '</span>';
      panel.appendChild(label);
    });
    menu.appendChild(btn); menu.appendChild(panel); toolbar.querySelector(".right").appendChild(menu);
    btn.addEventListener("click", function (e) { e.stopPropagation(); menu.classList.toggle("open"); });
    panel.addEventListener("change", function (e) {
      if (!e.target.matches("input[data-col]")) return;
      var idx = Number(e.target.dataset.col), show = e.target.checked;
      table.querySelectorAll("tr").forEach(function (tr) { if (tr.children[idx]) tr.children[idx].style.display = show ? "" : "none"; });
    });
    document.addEventListener("click", function (e) { if (!menu.contains(e.target)) menu.classList.remove("open"); });
  }
  ready(function () {
    document.querySelectorAll("table").forEach(function (table) {
      // ISFC36: table tools are opt-in. Add data-isfc-tools="true" only on tables where you want Search/Print/CSV/Columns.
      if (table.dataset.isfcTools !== "true" || table.dataset.isfcEnhanced === "1" || table.closest(".no-isfc-table-tools")) return;
      table.dataset.isfcEnhanced = "1"; table.classList.add("table", "align-middle");
      var wrap = table.closest(".table-responsive,.pis-table-wrap,.isfc-table-wrap");
      if (!wrap) { wrap = document.createElement("div"); wrap.className = "table-responsive"; table.parentNode.insertBefore(wrap, table); wrap.appendChild(table); }
      var dataRows = Array.from(table.querySelectorAll("tbody tr")).filter(function (tr) {
        var cells = Array.from(tr.children), txt = clean(tr.innerText).toLowerCase();
        if (!cells.length) return false;
        if (cells.length === 1 && cells[0].hasAttribute("colspan")) return false;
        if (/^no\s|no .*found|no .*yet|empty|not found/.test(txt)) return false;
        return true;
      });
      var toolbar = document.createElement("div");
      toolbar.className = "isfc-toolbar" + (table.dataset.isfcSearch === "false" ? " no-search" : "") + (table.dataset.isfcPrint === "false" ? " no-print" : "") + (table.dataset.isfcCsv === "false" ? " no-csv" : "");
      toolbar.innerHTML = '<div class="left"><span class="isfc-chip"><span data-feather="database"></span> ' + dataRows.length + ' rows</span></div><div class="right"><input class="form-control form-control-sm isfc-table-search" placeholder="Search this table"><button type="button" class="isfc-export-btn" data-action="print">Print</button><button type="button" class="isfc-export-btn" data-action="csv">CSV</button></div>';
      wrap.parentNode.insertBefore(toolbar, wrap);
      var input = toolbar.querySelector("input");
      if (input) input.addEventListener("input", function () { var q = input.value.toLowerCase(); table.querySelectorAll("tbody tr").forEach(function (tr) { tr.style.display = tr.innerText.toLowerCase().indexOf(q) > -1 ? "" : "none"; }); });
      var csvBtn = toolbar.querySelector('[data-action="csv"]');
      if (csvBtn) csvBtn.addEventListener("click", function () { download((document.title || "isfc-table").toLowerCase().replace(/[^a-z0-9]+/g, "-") + ".csv", tableToCsv(table), "text/csv;charset=utf-8"); });
      var printBtn = toolbar.querySelector('[data-action="print"]');
      if (printBtn) printBtn.addEventListener("click", function () { var title = document.title || "ISFC Table"; var cloned = table.cloneNode(true); var skip = hiddenColumnIndexes(table); cloned.querySelectorAll("tr").forEach(function (tr) { Array.from(tr.children).forEach(function (td, i) { if (skip.indexOf(i) !== -1) td.remove(); }); }); var html = '<!doctype html><html><head><title>' + title + '</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#102542}h2{margin:0 0 16px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #d8e2ef;padding:8px;text-align:left}th{background:#eef5fb}</style></head><body><h2>' + title + '</h2>' + cloned.outerHTML + '</body></html>'; var w = window.open('', '_blank'); if (w) { w.document.open(); w.document.write(html); w.document.close(); w.focus(); setTimeout(function(){ w.print(); }, 300); } });
      buildColumnMenu(table, toolbar);
      if (window.feather) { try { window.feather.replace(); } catch(e){} }
    });
    if (window.Choices) document.querySelectorAll("select:not([data-no-choices])").forEach(function (s) { try { if (!s.dataset.choice) { new Choices(s, { searchEnabled: true, shouldSort: false, itemSelectText: "" }); s.dataset.choice = "1"; } } catch(e){} });
    if (window.flatpickr) document.querySelectorAll('input[type="date"]').forEach(function (i) { try { flatpickr(i, { dateFormat: "Y-m-d", allowInput: true }); } catch(e){} });
    if (window.echarts) {
      document.querySelectorAll('[data-chart="donut"]').forEach(function (el) { var c = echarts.init(el); var labels=(el.dataset.labels||"Planned,In Process,Done").split(","); var values=(el.dataset.values||"35,25,40").split(",").map(Number); c.setOption({tooltip:{trigger:"item"},legend:{bottom:0},series:[{type:"pie",radius:["52%","72%"],center:["50%","45%"],avoidLabelOverlap:true,data:labels.map(function(l,i){return {name:l,value:values[i]||0};})}]}); window.addEventListener("resize",function(){c.resize();}); });
      document.querySelectorAll('[data-chart="bar"]').forEach(function (el) { var c = echarts.init(el); var labels=(el.dataset.labels||"Orders,BOM,Store,QC,Dispatch").split(","); var values=(el.dataset.values||"12,18,9,7,5").split(",").map(Number); c.setOption({tooltip:{trigger:"axis"},grid:{left:30,right:12,top:20,bottom:40},xAxis:{type:"category",data:labels,axisLabel:{interval:0,rotate:20}},yAxis:{type:"value"},series:[{type:"bar",data:values,barWidth:"45%"}]}); window.addEventListener("resize",function(){c.resize();}); });
    }
  });
})();


// ===== ISFC36 universal navbar search, toast and no-access UX =====
(function(){
  function ready(fn){ if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',fn); else fn(); }
  const pages=[
    ['Dashboard','/dashboard','dashboard command center orders KPI'],['Relationship Map','/relationship-map','document flow map'],['Reports Center','/reports','reports excel pdf csv'],
    ['Upload Master Data','/masters/upload','master upload template archive'],['Chefs','/chefs','chef master'],['Brands','/brands','brand master'],['Suppliers','/suppliers','supplier master'],['Customers','/customers','customer master'],['Inventory Master','/inventory','inventory ingredients items'],
    ['Recipes & Costing','/recipes','recipe costing bom'],['Prepare Recipe','/recipes/prepare','manual recipe'],['Customer / Internal Order','/orders/portal','customer order portal'],['Production Orders','/production/orders','production order register'],['Head Chef Planning','/production/head-chef','chef planning schedule'],['Store Issuance','/production/store-issuance','store issue material'],['Kitchen Summary','/production/kitchen-summary','kitchen all section summary'],['Thawing','/production/section/Thawing','thawing section'],['Cutting','/production/section/Cutting','cutting section'],['Butchery','/production/section/Butchery','butchery section'],['Marination','/production/section/Marination','marination section'],['Hot Kitchen','/production/section/Hot-Kitchen','hot kitchen section'],['Cold Kitchen','/production/section/Cold-Kitchen','cold kitchen section'],['Bakery/Pastry','/production/section/Bakery-Pastry','bakery pastry section'],['Quality Control','/qc','qc quality'],['Trayline / Packing','/packing','packing trayline'],['Dispatch / Delivery','/dispatch','dispatch delivery'],['Users & Access','/admin/users','user access permissions'],['Settings','/settings','system settings']
  ];
  function toast(type,title,msg){
    let t=document.querySelector('.isfc-toast'); if(!t){t=document.createElement('div');t.className='isfc-toast';document.body.appendChild(t)}
    t.className='isfc-toast show '+(type||''); t.innerHTML='<strong>'+ (title||'Notice') +'</strong><div class="small mt-1">'+(msg||'')+'</div>';
    setTimeout(()=>t.classList.remove('show'),5000);
  }
  window.isfcToast=toast;
  function wireSearch(input){
    if(!input || input.dataset.isfcSearchWired) return; input.dataset.isfcSearchWired='1';
    let box=input.closest('.search-box,.navbar-top-search-box,.position-relative')||input.parentElement;
    let menu=box.querySelector('.dropdown-menu');
    if(!menu){menu=document.createElement('div');menu.className='dropdown-menu border start-0 py-0 overflow-hidden w-100';box.appendChild(menu)}
    function render(){
      const q=(input.value||'').trim().toLowerCase();
      if(!q){box.classList.remove('isfc-search-open'); menu.classList.remove('show'); menu.innerHTML=''; return;}
      const hits=pages.filter(p=>(p[0]+' '+p[2]).toLowerCase().includes(q)).slice(0,8);
      menu.innerHTML='<h6 class="dropdown-header text-body-highlight fs-10 py-2">Search Results</h6>'+ (hits.length? hits.map(p=>'<a class="dropdown-item py-2" href="'+p[1]+'"><span class="fa-solid fa-link me-2"></span><span class="title">'+p[0]+'</span></a>').join('') : '<div class="px-3 py-3 text-body-tertiary">No project page found.</div>');
      box.classList.add('isfc-search-open'); menu.classList.add('show');
    }
    input.addEventListener('input',render);
    input.addEventListener('focus',render);
    input.addEventListener('keydown',function(e){ if(e.key==='Enter'){ const q=(input.value||'').trim().toLowerCase(); const hit=pages.find(p=>(p[0]+' '+p[2]).toLowerCase().includes(q)); if(hit){e.preventDefault(); location.href=hit[1];}} if(e.key==='Escape'){input.value='';render();}});
    document.addEventListener('click',e=>{if(box&&!box.contains(e.target)){box.classList.remove('isfc-search-open');menu.classList.remove('show')}});
  }
  ready(function(){
    document.querySelectorAll('.navbar-top-search-box input,.search-box input[type="search"],.navbar-top input[type="search"],input.search-input').forEach(wireSearch);
    const params=new URLSearchParams(location.search); if(params.get('toast')) toast(params.get('toast'),params.get('title'),params.get('msg'));
    document.querySelectorAll('[data-requires-access="false"],[aria-disabled="true"].btn').forEach(el=>{el.addEventListener('click',e=>{e.preventDefault();toast('warning','No Access','You do not have permission for this action. Ask Admin/Super Admin to update your access matrix.');});});
  });
})();
